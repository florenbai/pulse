<template>
  <a-drawer
    ...
    @cancel="handleCancel"
  >
    <template #title> 快速告警静默规则配置 </template>
    <div>
      <a-alert>快速静默将会在降噪配置中。</a-alert>
    </div>
    <a-form
      ref="formRef"
      layout="vertical"
...
<script setup lang="ts">
  import { SilenceRequest, submitSilenceForm } from '@/api/alert-denoise';
  ...
  const show = (tags: Map<string, string> | undefined, alertTitle: string) => {
    if (tags !== undefined) {
