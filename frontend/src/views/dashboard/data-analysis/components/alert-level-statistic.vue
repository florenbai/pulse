<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card class="general-card" :header-style="{ paddingBottom: '16px' }">
      <template #title> 告警等级统计 </template>
      <template #extra>
        <StatisticTimeSelect
          fault="最近7天"
...
<script lang="ts" setup>
  ...
  import {
    AlertLevelDistributionRecord,
    TopAlertParams,
    queryAlertLevelStatistic,
  } from '@/api/alert-statistic';

  const { loading, setLoading } = useLoading(true);
  const chartTitleData = ref<string[]>([]);
  const chartValueData = ref<any[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const { data } = await queryAlertLevelStatistic(listParams.value);

      data.forEach((el: AlertLevelDistributionRecord) => {
        chartTitleData.value.push(el.levelName);
        chartValueData.value.push({
          value: el.count,
          itemStyle: {
            color: el.color,
          },
